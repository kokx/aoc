...
for i in range(0, len(lines), 4):
    ...
    for attempt in operations:
        ...
        if test == after:
            goodones.add(attempt)

    actualops[op[0]] = set.intersection(actualops[op[0]], goodones)

    #if len(goodones) == 0:
    #    print('Not good', op)
    #if len(goodones) == 1:
    #    print('Actual OP %d' % op[0], goodones[0])
    #    actualops[op[0]] = goodones[0]

for i in range(0, 16):
    for k, v in actualops.items():
        if len(v) == 1:
            # remove this from everyone else
            for k1, v1 in actualops.items():
                if k1 != k:
                    actualops[k1] = set.difference(actualops[k1], actualops[k])
...
with open('program') as f:
    ...
    for line in lines:
        mop = p.match(line.strip())
        op = [int(mop.group(1)), int(mop.group(2)), int(mop.group(3)), int(mop.group(4))]
        finalops[op[0]](registers, op[1], op[2], op[3])
