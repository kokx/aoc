...
def bfs_paths(grid, start, goals):
    ...
    while queue:
        ...
        for next in set(get_neighbors(grid, vertex)) - set(path):
            if next in goals:
                yield path + [next]
            else:
...
for number_round in range(0, 300):
            ...
            if grid[i][j] == 'G':
                ...
                if len(enemies) > 0:
                    ...
                    if len(e_neighbors) > 0:
                        paths = bfs_paths(grid, (i, j), e_neighbors)
                ...
                if len(enemies) > 0:
                    ...
                    if len(g_neighbors) > 0:
                        paths = bfs_paths(grid, (i, j), g_neighbors)
                        best = find_best_paths(paths)
                        if len(best) > 0:
                            next = find_next(best)
